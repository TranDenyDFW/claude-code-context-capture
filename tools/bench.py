...
def measure(session_id=None, repeats=REPEATS):
...
def regressions(base, now):
    """Which tabs got slower by BOTH thresholds. Returns a list of human-readable lines."""
    bad = []
    for tab, was in base.get("tabs", {}).items():
        ...
        delta = after - before
        if delta > SLOWER_BY_MS and before > 0 and delta / before > SLOWER_BY_FRACTION:
            bad.append(f"{tab}: {before:.0f} ms -> {after:.0f} ms "
                       f"(+{delta:.0f} ms, +{delta / before * 100:.0f}%)")
    return bad
...
def self_test():
    ...
    cases = [
        ("an unchanged run is not a regression", regressions(base, same) == []),
        ...
        ("a tab that stopped existing is a regression",
         len(regressions(base, {"tabs": {"slow": {"warm_ms": 1000.0}}})) == 1),
        ...
        ("but a fast tab genuinely blowing up still fails (12 -> 900 ms)",
         len(regressions({"tabs": {"d": {"warm_ms": 12.0}}},
                         {"tabs": {"d": {"warm_ms": 900.0}}})) == 1),
        ("both thresholds are required, not either",
         regressions(base, {"tabs": {"slow": {"warm_ms": 1040.0},   # +40 ms, +4%
                                     "fast": {"warm_ms": 30.0}}}) == []),   # +14 ms, +88%
